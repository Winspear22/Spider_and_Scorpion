...
bool ImageDownloader::downloadImage(const std::string &url, const std::string &outputDir)
{
	...
	{
		std::cerr << "Error: Nom de fichier suspect (path traversal) → " << filename << std::endl;
		return FAILURE;
	}
	...
	{
    	std::cerr << "Error: Nom de fichier trop long → " << filename.substr(0, 50) << "..." << std::endl;
    	return FAILURE;
	}
	...
	if (!file.is_open())
	{
    	std::cerr << "Error: Impossible d'ouvrir le fichier " << fullPath << std::endl;
    	return FAILURE;
	}
...
size_t ImageDownloader::_writeToFile(void *ptr, size_t size, size_t nmemb, void *stream)
{
